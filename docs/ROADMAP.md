...
| # | 功能点 | 说明 |
|---|--------|------|
| 1 | `class_name Logger extends Node` | Autoload 注册到 project.godot |
| 2 | 四级日志：`debug()` `info()` `warn()` `error()` | 低于设定级别的自动跳过 |
| 3 | 控制台彩色输出 | `print_rich` 带颜色区分级别 |
| 4 | 写入日志文件 | `user://logs/game_时间戳.log` |
| 5 | 自动创建 `logs/` 目录 | `DirAccess.make_dir_recursive_absolute` |
| 6 | 保留最近 5 个日志文件 | 启动时自动清理旧文件 |
| 7 | `log_level` 从 `config.json` 读取 | 不存在时默认 `debug` |
| 8 | `get_recent(n)` 返回最近 N 条 | 供 DebugConsole 调用 |
| 9 | `log_added` 信号 | 实时推送新日志条目 |
| 10 | ERROR 级别自动 `push_error` | 调试构建中弹出断点 |
...
| # | 功能点 | 说明 |
|---|--------|------|
| 1 | `Easing` | 缓动函数：`ease_out_quad` `ease_out_bounce` `ease_out_elastic` `ease_in_out_cubic` 等 10+ 个 |
...
**完成标志**：创建 `config.json` 写入 `{"debug":{"log_level":"info"}}`，`ConfigManager.get_value("debug.log_level","debug")` 返回 `"info"`。
...
| 3 | 按序调用各子系统的 `initialize()` | 顺序 = autoload 声明顺序即可，暂不需要拓扑排序 |
...
| 9 | `pause(layer, source)` → 返回 `int` | 多层暂停栈 |
...
| 13 | `set_title()` `set_content()` `on_confirm()` `on_cancel()` `show()` | 链式调用 |
...
| 2 | `is_action_just_pressed(action, buffer_frames)` | 刚按下 + 缓冲 |
| 3 | `is_action_just_released(action)` | 刚释放 |
| 4 | `get_action_strength(action)` | 按压强度 |
...
| 13 | `get_value(name)` | 计算最终值 |
| 14 | `add_bonus(name, flat, mult, source_id)` | 添加加成 |
| 15 | `remove_bonus(name, source_id)` | 移除加成 |
...
**完成标志**：创建 Orc(hostile_to: [Player])，Player 攻击 Orc，伤害递减，Orc 生命归零触发 `die()`。
...
| 3 | `add_transition(from, to, condition, priority)` | 条件转换 |
...
| 5 | `use(caster, target)` | 消耗资源 → 冷却 → 执行 |
...
| 10 | `duration` `max_stacks` `stack_policy` | 叠加策略 |
| 11 | `attribute_modifiers` | 属性修改字典 |
...
| 15 | `apply_buff(template)` | 同名处理（刷新/叠加/拒绝） |
...
| 2 | `target` 跟随 | `@export var target: Node2D` |
| 3 | LERP / SNAP / SMOOTH_DAMP 三种模式 | `follow_mode` 枚举 |
...
| 5 | 语言从 ConfigManager 读取 | 默认 `zh_CN` |
...
| 3 | 内建 `help` `clear` `fps` `list_nodes` | 基础命令 |
| 4 | 命令历史 | ↑↓ 浏览历史 |
